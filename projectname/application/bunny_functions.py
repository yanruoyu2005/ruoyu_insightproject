...
def test_single(number):
    # This function is for forecasting existing companies when user enter an existing numeric ID.    
...
def generate_array():
    # Generate a strategy array represent all possible combinations of factors for selected factors
    #path1 = get_script_path() # previously used for local server
    path1 = '/home/ubuntu/application' # used for production server
...
def similarity(whole_feature,company_index):
    # Propose similar competitors for the user.
    path_data = path1 + '/exploratory_data'
